...
class SetUpTest(TestCase):
    client = Client()
    data = {"title": "test_book", "pace": "10", "category": "all", "sort": "relevance"}
    item = {
        "id": "test_id",
        "volumeInfo": {
            "authors": ["test_author"],
    ...
    def setUp(self, mock_request=None):
        mock_request.return_value = {"totalItems": 100, "items": [self.item]}
...
class TestAPI(SetUpTest):
    def test_fetch_to_session(self):
        ...
        self.assertEqual(
            json.loads(self.client.session["test_id"])["title"], "test_book"
        )
